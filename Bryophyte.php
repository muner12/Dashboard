...
if(isset($_POST['insert'])){
   ...
   // $botincal=$_POST[''];
    ...
    $check_recored=mysqli_query($con,"select * from bryophytes where botanical='$botincal'");
...
$fetch_one_row=[];
if(isset($_GET['type']) && $_GET['type']=='update'){
    $fetch_one_row=mysqli_fetch_assoc(mysqli_query($con,"select * from bryophytes where id={$_GET['id']}"));
...
if(isset($_POST['update'])){
   ...
   // $botincal=$_POST[''];
    ...
    $sql="UPDATE `bryophytes` SET `botanical` = '$botincal', `synonyams` = '$sysnonyams',
     `family` = '$family', `group` = '$group', `country` = '$country',
      `province` = '$province', `collector` = '$collector', `collection_number` = '$collection_number', `year` = '$year',
       `upload_date` = '$upload_date', `description` = '$desciption',
        `picture` = '$picture_name' WHERE `bryophytes`.`id` = {$_GET['id']}";
        $result=mysqli_query($con,$sql);
