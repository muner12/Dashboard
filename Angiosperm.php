...
if(isset($_POST['insert'])){
   ...
   // $botincal=$_POST[''];
    ...
    $check_recored=mysqli_query($con,"select * from angiosperms where botanical='$botincal'");
...
$fetch_one_row=[];
if(isset($_GET['type']) && $_GET['type']=='update'){
    $fetch_one_row=mysqli_fetch_assoc(mysqli_query($con,"select * from angiosperms where id={$_GET['id']}"));
...
if(isset($_POST['update'])){
   ...
   // $botincal=$_POST[''];
    ...
    $sql="UPDATE `angiosperms` SET `botanical` = '$botincal', `synonyams` = '$sysnonyams',
     `family` = '$family', `group` = '$group', `country` = '$country',
      `province` = '$province', `collector` = '$collector', `collection_number` = '$collection_number', `year` = '$year',
       `upload_date` = '$upload_date', `description` = '$desciption',
        `picture` = '$picture_name' WHERE `angiosperms`.`id` = {$_GET['id']}";
        $result=mysqli_query($con,$sql);
        ...
        <form method="post" enctype="multipart/form-data">
            ...
            <div class="row d-flex justify-content-center">
                ...
                <div class="col-lg-5">
                    ...
                    <?php
                    if(isset($_GET['type']) && $_GET['type']=='update'){
                        echo "<img width='100px' height='60px' src='upload/bryophytes/{$fetch_one_row['picture']}'>";
                    }
                    ?>
